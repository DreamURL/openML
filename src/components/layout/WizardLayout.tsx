...
import { AdUnit, SideAd } from '@/components/ad/AdUnit'
...
export function WizardLayout() {
  ...
  const handleStepClick = (step: 1 | 2 | 3) => {
    goToStep(step)
    navigate({ pathname: stepRoutes[step - 1], search: location.search })
  }
  ...
  const handleNext = () => {
    if (activeStep < 3 && canGoNext()) {
      ...
      goToStep(nextStep)
      navigate({ pathname: stepRoutes[nextStep - 1], search: location.search })
    }
  }
  ...
  const handleBack = () => {
    if (activeStep > 1) {
      ...
      goToStep(prevStep)
      navigate({ pathname: stepRoutes[prevStep - 1], search: location.search })
    }
  }

  return (
    <>
      <aside className="hidden xl:flex fixed left-0 top-0 h-screen w-[176px] items-center justify-center z-10">
        <SideAd slot="8504504766" />
      </aside>

      <aside className="hidden xl:flex fixed right-0 top-0 h-screen w-[176px] items-center justify-center z-10">
        <SideAd slot="8504504766" />
      </aside>

      <div className="flex flex-col min-h-screen xl:px-[176px]">
        {privacyOpen && (
